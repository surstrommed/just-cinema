...
import { LOCAL_STORAGE_ACTOR_SORT } from "constants/common";
...
export const ActorsSorting: FC<ISorting> = ({ dataLength }) => {
  const { actorSort, handleSort } = useContext(SortModeContext);
  ...
  const [sortType, setSortType] = useState<SortType>(actorSort.sortType);
  const [sortOrder, setSortOrder] = useState<SortOrder>(actorSort.sortOrder);
  ...
  const handleSubmit = () => {
    ...
    handleSort(LOCAL_STORAGE_ACTOR_SORT, sortType, sortOrder);
  };
      ...
      {(pageNumber === 1 && dataLength > 1) || pageNumber !== 1 ? (
        <Box sx={styles.main}>
          ...
          <Dialog open={open} onClose={handleClose}>
            <DialogTitle>Choose actor sorting</DialogTitle>
            <DialogContent>
              <Box component="form" sx={styles.dialogContent}>
                <FormControl sx={styles.formControl}>
                  ...
                  >
                    ...
                    <MenuItem value="birthDay">By birth date</MenuItem>
